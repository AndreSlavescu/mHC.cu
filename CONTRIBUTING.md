...
Generated files go to `src/csrc/benchmarks/bench_<name>.cu` and start with `// AUTO-GENERATED by benchgen.py`. Do not edit them manually. The three struct-based benchmarks (`bench_fused_rmsnorm_matmul.cu`, `bench_fused_rmsnorm_matmul_backward.cu`, `bench_mhc_layer.cu`) are hand-written and not managed by benchgen.
